...
$.extend({
	//打开图片查看器
	openPhotoGallery : function(obj,iframeUrl){
	    var $img = $(obj),
  			imgUrl = $img.attr('data-url');//$img[0].src;
		if(!imgUrl) return;
		//HTML5提供了一个新属性naturalWidth/naturalHeight可以直接获取图片的原始宽高
		var img = $img[0],
			imgHeight = img.naturalHeight * 3,
			imgWidth = img.naturalWidth  *3,
		...
		$gallerys.find(".gallery-pic").each(function(i, elem){
		    var url = $(this).attr('data-url'),
				img = $(this)[0],
				nH = img.naturalHeight * 3,
				nW = img.naturalWidth * 3,
				ratio  = nW / nH,
				w = nW,
				h = nH;
		...
		$("<iframe></iframe").appendTo("body")
			...
			.css({
				...
				top : 0,//(document.body.clientHeight - winHeight) /2,
				width : winWidth,
				height : winHeight,
